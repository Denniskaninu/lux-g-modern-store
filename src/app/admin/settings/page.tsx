...
import { uploadImage, resetSalesData } from "@/lib/data";
...
import { Loader2, Camera, TriangleAlert } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const handleResetSales = async () => {
    ...
    try {
        ...
        toast({
            title: "Success!",
            description: "All sales data has been reset.",
        });
        // You might want to refresh some data or redirect here
    } catch (error: any) {
        toast({
            variant: "destructive",
            title: "Reset Failed",
            description: error.message || "Could not reset sales data.",
        });
    } finally {
        setResetting(false);
    }
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                This action cannot be undone. This will permanently delete all sales
                                records and reset your revenue and profit tracking to zero.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={handleResetSales}>
                                Yes, reset all sales data
                            </AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
                <p className="text-sm text-muted-foreground mt-2">
                    This will delete all sales history. Product inventory will not be affected.
                </p>
            </CardContent>
        </Card>
    </div>
